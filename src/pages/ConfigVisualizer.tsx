...
const renderHTTPLBContent = () => {
    ...
    // Note: The original code relied on variables calculated in the main scope.
    // We must re-calculate them here for this isolated function to work.
    let lbType = 'HTTP';
    let lbTypeClass = 'bg-slate-600';
    if (spec?.https_auto_cert) {
      lbType = 'HTTPS (Auto Cert)';
                          ...
                          .map(([name, waf]) => {
                            ...
                            return (
                              <div key={name} className="p-5 bg-slate-800/40 rounded-lg border border-slate-700/30">
                                ...
                                {/* Rest of WAF details... (omitted for brevity but logic is same as above) */}
                              </div>
                            );
                          })}
                      </div>
